...
class BiSeNetMultiLevel(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        # upsampling
        cx1_up = torch.nn.functional.interpolate(
            cx1, size=sx.size()[-2:], mode="bilinear"
        )
        cx2_up = torch.nn.functional.interpolate(
            cx2, size=sx.size()[-2:], mode="bilinear"
        )
        cx = torch.cat((cx1_up, cx2_up), dim=1)

        # Always compute supervision outputs
        cx1_sup = self.supervision1(cx1)
        cx2_sup = self.supervision2(cx2)

        # Upsample supervision outputs for loss calculation
        cx1_sup_up = torch.nn.functional.interpolate(
            cx1_sup, size=input.size()[-2:], mode="bilinear"
        )
        cx2_sup_up = torch.nn.functional.interpolate(
        ...
        # upsampling final output
        result = torch.nn.functional.interpolate(
            result, scale_factor=8, mode="bilinear"
        )
        result = self.conv(result)

        # Always return the main output and both supervision outputs
        return result, cx1_sup_up, cx2_sup_up
